...
1. **现成可交付**（下载 `.app` 就能用）：**只有 Apple Silicon Mac，macOS 11+**。
   这是 AgentEar 这个**成品**的限制。
2. **runtime 层是通的**：Linux x64/arm64、Windows x64 都有官方产物。
   客户是 Linux 服务器时，路存在 —— 但**要我们自己组装**，AgentEar 的 app 壳不能用。
3. **Intel Mac 无解**：上游从 `v0.1.9` 到 `v0.2.6` 就没发过 macOS x64。
   就算主程序编成通用二进制，**ASR 子进程照样起不来**。
