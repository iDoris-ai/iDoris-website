...
| 组件 | 一句话 | 状态 | 详设 |
|:---|:---|:---|:---|
| **Voice** | 说话变文字。**泰/英/中分别可用；一句话里夹着说（code-switch）目前还不行** —— 实测夹英文 CER 31.1%，见 [`voice.md`](voice.md) §4.1 | **[已核 2026-09-07] 基础在 `iDoris-ai/AgentEar`** —— 有官方 `.app`，但只覆盖 Apple Silicon Mac（§5）| [`voice.md`](voice.md) |
...
**例外（有条件，不是无条件）**：数据敏感度高的客户，**在 Apple Silicon Mac 上**，
Voice 可以单独部署在他们机器上 —— 这正是选本地 ASR 而非云 API 的原因。

> 🔴 **BD 对客户就照下面这样说，别只说前半句**（2026-09-07）：
>
> | 客户的机器 | 能说的话 | 我们的证据 |
> |:---|:---|:---|
> | **Apple Silicon Mac**（M1+ / macOS 11+）| 「**可以，但要先做一次安装验证**」| 有官方 `.app`；**我们自己还没装过** |
> | Linux / Windows **x64** | 「**目前不承诺。** 上游有运行时产物，但我们没有组装和验证过可交付形态」| 未实测 |
> | **Intel Mac** | 「**我们查过的版本里没有官方 Intel 运行时**；能不能自己编，我们没验过」| 仅查了两个发布 tag |
>
> **三条措辞纪律**：
> 1. **不要说「开箱即用」** —— 我们没安装实测过，说了客户当场装不上就是我们的信用。
> 2. **Linux/Windows 那一行不是软承诺**，是明确的「现在不承诺」。写成「路存在」
>    会被客户听成「你们能做，只是还没做」。
> 3. **不要说 Intel「不行」**，说「没有官方产物、自建未验证」——
>    我们查的是两个发布 tag，不是穷举。
>
> 详见 [`voice.md`](voice.md) §7。
