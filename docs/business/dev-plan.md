...
| Whisper large-v3 | **[已核 2026-09-05]** `Systran/faster-whisper-large-v3` = MIT | ~~阻塞解除~~ |
...
| **Voice 上游停更** | **[已核] 已发生** | GPU 路径可能跑不起来 | `faster-whisper` 维护者停更 9 个多月而后端仍在发版。**同时锁两个包的版本 + 部署时显式验 GPU 路径**（报错在 import 阶段不出现）|
