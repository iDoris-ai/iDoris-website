...
| **SenseVoiceSmall + whisper.cpp**（经 AgentEar）| Apache-2.0 / MIT | ✅ | 语音转写 | **[已核 2026-09-07] 已替换 faster-whisper。** 本地跑是卖点不是成本；⚠️ 官方产物只覆盖 Apple Silicon Mac |
...
| 权重 | 状态 | 阻塞什么 |
|:---|:---|:---|
| 图像生成模型 | **[待核]** —— 尚未选定具体模型 | **Creative 图像部分不对外交付** |
| ~~Whisper large-v3~~ | **[已核 2026-09-05] 历史项，已不适用** —— 我们不再用 faster-whisper 链路。现行权重许可见 `deployment-runbook.md` §2.3 | ~~阻塞解除~~ |
| Docling 自带模型 | **[已核 2026-09-05]** CDLA-Permissive-2.0 + Apache-2.0 / MIT | ~~阻塞解除~~（条件:不启用外部 OCR 引擎）|
...
| 步 | 做什么 | 用什么 | 产出 |
|:--|:---|:---|:---|
| ~~A1~~ | ~~Voice 现状盘点~~ | — | ✅ **[已核 2026-09-07] 已完成**，见 `verification-2026-09-07-voice-v0.md`。**接替它的是「装一次 AgentEar 做实测」** —— 目前所有「跑得起来」的结论都只是自述 |
| A2 | Gateway 最小版 | LiteLLM | 统一入口 + 三档路由 + 审计写 Postgres |
| A3 | Voice 泰语评测 | whisper.cpp `distill` q5_0 | 20 段**真实场景**音频的 CER 报告，**按场景分层**。⚠️ AgentEar 已有朗读语料的数（6.08%，且 FLEURS 有污染）——**我们要补的是会议室/电话/噪声那一层** |

~~**A1 必须最先做。**~~ **[已核 2026-09-07] A1 已完成** —— 假设成立，但技术栈四项全错。
**接替它的首要项是首次实装实测**：不能在「别人文档说能跑」上继续叠设计。

### 阶段 B — 一条端到端（第一个能演示的东西）

| 步 | 做什么 | 用什么 | 产出 |
|:--|:---|:---|:---|
| ~~B1~~ | ~~LangGraph 离线验证~~ | — | **[已核 2026-09-05]** 零出网(不设变量时)。改为:部署清单 unset + **启动断言** |
| B2 | Assistant 骨架 | LangGraph + Postgres | 通用流程骨架 + 审批队列接口 |
| B3 | 「会议→纪要→任务」 | Voice + Assistant | 一条可演示的端到端 |

**B1 已解除**（[已核 2026-09-05]，报告见 `verification-2026-09-05.md`）。
socket 层拦截 + 正对照证明：不设任何环境变量时 **0 次外部连接**；
但设 `LANGSMITH_TRACING=true` 时**确实会连** `api.smith.langchain.com`。

**所以风险不在库，在部署配置。** 控制手段是机械的：部署清单 unset 那几个变量、
**启动时断言检测到就拒绝启动**、客户环境部署前跑一次验证脚本。
原定的自建状态机降级路径**可以取消**。
...
| 语音 | **SenseVoice + whisper.cpp（本地，经 AgentEar）** | 云 API | 泰语数据敏感度高，**本地跑是卖点不是成本**。⚠️ 但只在 Apple Silicon Mac 上有官方产物 |
...
| 风险 | 概率 | 影响 | 对策 |
|:---|:---:|:---|:---|
| ~~LangGraph 无法完全离线~~ | — | — | **[已核 2026-09-05] 已解除。** 不设变量时零出网；设 `LANGSMITH_TRACING=true` 时确实会连。**风险不在库、在部署配置** —— 对策是启动断言（`products/assistant/egress_guard.py`，检测到就**拒绝启动**）+ 部署清单。**自建状态机的降级路径已取消**（本文档 §「阶段 B」同款结论，两处不要再打架）|
| 图像权重不可商用 | 中 | Creative 图像砍掉 | 先只上 `copy` |
| ~~Voice 现状与假设不符~~ | — | — | **[已核 2026-09-07] 已发生并已处理**：假设成立但技术栈四项全错，相关文档已重写 |
| **Voice 上游产物断供** | 中 | 主链路装不起来 | **[已核 2026-09-07] 风险换了对象**：不再是「库停更」，是上游 `modelscope/FunASR` **不再发 runtime 产物**（我们取的是发布产物，不是自己编译）。对策：锁 tag + **自留产物副本**（版本号挡不住上游删 release）|
