...
### 第 9 条展开：写任何检查之前必过的一道问题

前八条管的是**事实**和**流程**。第 9 条管的是**检查本身可不可信** ——
这一条是 2026-09-06/07 那一晚反复栽出来的，横跨两种形态：

**① 断言错了**（通过了，但通过的路径不是你以为的那条）

- `compare` 的「重复配对」变异生效却没被抓到 —— `CoverageError` 是
  `CompareRejected` 的子类，测试被**后面**的覆盖对账接住了，`expect_raises` 分不出来
- `search` 的「无出处答案」被**数字校验**接住 —— 换成不含数字的答案就直接放行，
  而合同问答里不含数字的答案很常见（期限、机构、语言、责任方）

**② 检查不承重**（跑了，但去掉它什么都不会变）

- `check-letters` 的「剥 HTML 注释」那一步：去掉它整套自检**全绿**

**②那个不是样本不够，是量纲反了** ——
判据全写成「**至少**有 N 个」，而想抓的错误方向是「**你多算了**」。
一个「至少」型的判据永远测不出「多算」，所以那格声称验证剥注释的正对照
**从一开始就不可能承重**。
...
**把它变成每次都要过的一道问题，而不是「记住上面这三个案例」** ——
否则下次换个形态的坑照样掉。

（这条的最终形状来自 iDoris 侧的跨仓库评审：我提的是三个案例，
对方指出该写成设计要求。他们那边记为 FU-8。）
