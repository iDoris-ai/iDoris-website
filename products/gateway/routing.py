...
三条不可破的规则,全部由测试兜住(见 test_routing.py):
  ...
  2. 预算用尽时**必须**拒绝,不是降级后继续跑。
     ...
     **它和「敏感数据必须留本地」是同一类东西**(iDoris 侧评审时点明的归类):
     两个拒绝终态的共性是「**都不是重试或降级能解决的问题**」——
     一个是隐私约束,一个是商业约束。
     把预算塞进技术性降级链(忙/OOM 触发的那条),等于替客户决定
     「继续花,只是花得少些」—— 而客户设上限就是不想再花。
     悄悄降级的账单,客户看到时钱已经花了。

     **但「拒绝」的作用域要选对**(iDoris 契约 v1 §4 提出的细化):
     `paid_only` 只闸掉 `cost > 0` 的候选,零成本的本地模型照常可用;
     `all` 则一律拒绝。**我们选 paid_only**,理由是具体的 ——
     `policy.example.json` 里 `line.reply`(面向客人的 LINE 回复)本来就走 local,
     因为含客人信息。选 `all` 的话,老板的月度预算烧完 = **客人发消息没人回**;
     选 `paid_only` 则停掉的是文档翻译、对比这类内部批处理 ——
     那是看得见的、可以等到下次充值的。

     **前提是 402 必须传到人**:内部功能悄悄停掉而没人知道为什么,
     和悄悄花钱是同一类错误的两面。
